...
enum TpmsType {
    ...
    N,
	changekP,
	distanceBasedkP,  // 11: uses distance to surface
	distanceBasedkD   // 12: diamond with distance to surface
};
...
void generateTPMSwithPlate69(const std::string& input_path, const std::string& output_path)
{
	...
	// Build AABB tree for distance queries to the mesh surface
	...
	// Input tpms type
	std::cout << "Input TPMS type: 0-kP, 1-kD, 2-kG, 10-changekP, 11-distanceBasedkP, 12-distanceBasedkD" << std::endl;
	int tpms_input;
	std::cin >> tpms_input;
...
#pragma omp parallel for
	...
	const auto& tpms_function = [tpms_type, omega](double x, double y, double z, double dist_to_surface)-> double {
		double tpms_value = 0;
		...
		else if (tpms_type == distanceBasedkP){
			// Use distance to surface to modulate frequency
			// Keep omega constant for first 1 wavelength, then decrease
			// Wavelength = 2*pi/omega
			const double threshold = 2.0 * M_PI / omega;
			double w_dist;
			if (dist_to_surface < threshold) {
				w_dist = omega;  // constant near surface
			} else {
				// Start decreasing after threshold
				const double excess = dist_to_surface - threshold;
				w_dist = omega / sqrt(1.0 + 2*excess);
			}
			tpms_value = cos(w_dist * x) + cos(w_dist * y) + cos(w_dist * z);
		}
		else if (tpms_type == distanceBasedkD){
			// Diamond TPMS with distance-based frequency modulation
			// Starts decreasing immediately from surface (no threshold)
			const double w_dist = omega / sqrt(1.0 + 2*dist_to_surface);
			// Diamond function: cos(x)*cos(y)*cos(z) - sin(x)*sin(y)*sin(z)
			tpms_value = cos(w_dist * x) * cos(w_dist * y) * cos(w_dist * z) 
			           - sin(w_dist * x) * sin(w_dist * y) * sin(w_dist * z);
		}
	...
	// Precompute distances to surface if using distance-based TPMS
	std::vector<double> distances(grid.rows(), 0.0);
	if (tpms_type == distanceBasedkP || tpms_type == distanceBasedkD) {
...
#pragma omp parallel for
	for (int i = 0; i < grid.rows(); i++) {
		...
		double f = tpms_function(x, y, z, dist_to_surface);
...
int main(int argc, char** argv) {
    if (argc < 3) {
        std::cerr << "Usage: ./tpms_matan input.off output_name" << std::endl;
        return 1;
    }
    ...
    // Always save to tpms_models folder
